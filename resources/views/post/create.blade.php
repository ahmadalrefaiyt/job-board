<x-layout :title="$pageTitle" pageHeader="Create New Post">
                ...
                <div class="col-span-full">
                    ...
                    <div class="mt-2">
                        <textarea name="body" id="content" rows="3"
                            class="block w-full rounded-md bg-white px-3 py-1.5 text-base text-gray-900 outline-1 -outline-offset-1 {{ $errors->has('body') ? 'outline-red-500' : 'outline-gray-300' }}  placeholder:text-gray-400 focus:outline-2 focus:-outline-offset-2 focus:outline-indigo-600 sm:text-sm/6">{{ old('body') }}</textarea>
                    </div>
                    @error('body')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
